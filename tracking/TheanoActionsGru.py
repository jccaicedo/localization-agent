...
def initClassifier(stateDim, targetDim, zeroTailFc):
    if not zeroTailFc:
...
class TheanoGruRnn(object):
    ...
    def preprocess(self, data, boxes, flow):
        ...
        if self.useAttention == 'square':
            firstFrameMasks = VisualAttention.getSquaredMasks(data[:,0,...], boxes[:,0,:])
            data[:,0,...] *= firstFrameMasks #Multiplicative mask
        elif self.useAttention == 'squareChannel':
            b,t,c,w,h = data.shape
            firstFrameMasks = VisualAttention.getSquaredMaskChannel(data[:,0,...], boxes[:,0,:])
            data = NP.append(data, NP.zeros((b,t,1,w,h)), axis=2)
            data[:,0,-1,:,:] = firstFrameMasks
        # Generate labels
        labels = MoveBoxes.prepareTargets(boxes, self.imgSize)
    ...
    def fit(self, data, boxes, flow):
        data, boxes, labels = self.preprocess(data, boxes, flow)
        cost, bbox, actionProb = self.fitFunc(data, boxes[:, 0, :], labels)
        print ' * pred:',NP.argmax(actionProb[0,...],axis=1)
        return cost, self.postprocess(bbox)
      
    def forward(self, data, label, flow):
        data, boxes, label = self.preprocess(data, label, flow)
        cost, bbox, actionProb = self.forwardFunc(data, boxes[:, 0, :], label)
        print ' * pred:',NP.argmax(actionProb[0,...],axis=1)
        return cost, self.postprocess(bbox)
    ...
    def buildModel(self, batchSize, inputDim, stateDim, targetDim, zeroTailFc, initialLearningRate, use_cudnn, imgSize):
        ...
        if self.modelArch.endswith('ConvLayers'):
            ...
            def step(img, prev_bbox, prev_prob, state):
                img = attention(img, prev_bbox)
                features = buildCNN(img, self.cnn, convParams)
                h1 = gru(features, prev_bbox, state, Wr, Ur, br, Wz, Uz, bz, Wg, Ug, bg)
                actionProb = actionClassifier(h1, W_fc2, b_fc2)
                boxes = MoveBoxes.moveBoxes(prev_bbox, actionProb, imgSize)
                return boxes, actionProb, h1
        ...
        initProbs = Tensor.zeros((batchSize, targetDim))
        # Move the time axis to the top
        sc, _ = Theano.scan(step, sequences=[imgs.dimshuffle(1, 0, 2, 3, 4)], outputs_info=[starts, initProbs, state])
    
        bbox_seq = sc[0].dimshuffle(1, 0, 2)
        actionProb = sc[1].dimshuffle(1, 0 ,2)
    
        # targets: of shape (batch_size, seq_len, targetDim)
        targets = Tensor.imatrix('targets')
    
        # Negative log-likelihood
        y_given_x = actionProb.reshape( (actionProb.shape[0]*actionProb.shape[1],actionProb.shape[2]) )
        ys = targets.reshape( (targets.shape[0]*targets.shape[1],) )
        cost = -Tensor.mean(Tensor.log(y_given_x)[Tensor.arange(ys.shape[0]), ys])
        ...
        # Fit model function: starts are initial boxes, targets are expected actions
        fitFunc = Theano.function([imgs, starts, targets], [cost, bbox_seq, actionProb], updates=rmsprop(cost, params, learningRate), allow_input_downcast=True)
        forwardFunc = Theano.function([imgs, starts, targets], [cost, bbox_seq, actionProb], allow_input_downcast=True)
        '''
        imgStep = getTensor("images", Theano.config.floatX, 4)
        startsStep = Tensor.matrix()
        stateStep = Tensor.matrix()
        initProbStep = Tensor.matrix()
        stepFunc = Theano.function([imgStep, startsStep, initProbStep, stateStep], step(imgStep, startsStep, initProbStep, stateStep))
        '''
        stepFunc = None
    ...
    def init_params(self, inputDim, stateDim, targetDim, zeroTailFc):
        ...
        W_fc2, b_fc2 = initClassifier(stateDim, targetDim, zeroTailFc)
