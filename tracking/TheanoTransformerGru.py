...
class TheanoGruRnn(object):
    ...
    def buildModel(self, batchSize, inputDim, stateDim, targetDim, zeroTailFc, initialLearningRate, use_cudnn, imgSize):
        ...
        sc, _ = Theano.scan(step, sequences=[imgs.dimshuffle(1, 0, 2, 3, 4)], outputs_info=[start_params, start_prediction, state])
        ...
        fitFunc = Theano.function([seq_len_scalar, imgs, start_params, targets], [cost, bbox_seq], updates=rmsprop(cost, params, learningRate), allow_input_downcast=True)
        forwardFunc = Theano.function([seq_len_scalar, imgs, start_params, targets], [cost, bbox_seq], allow_input_downcast=True)
        imgStep = getTensor("images", Theano.config.floatX, 4)
        startsStep = Tensor.matrix()
        stateStep = Tensor.matrix()
        predStep = Tensor.matrix()
        stepFunc = Theano.function([imgStep, startsStep, predStep, stateStep], step(imgStep, startsStep, predStep, stateStep))
